package com.codeazur.as3swf.data
{
	...
	public class SWFShape
	{
		...
		protected var _fillStyles:Array;
		protected var _lineStyles:Array;
		...
		public function get fillStyles():Array { return _fillStyles; }
		public function get lineStyles():Array { return _lineStyles; }
		...
		protected function processSubPath(subPath:Array, lineStyleIdx:uint, fillStyleIdx0:uint, fillStyleIdx1:uint):void {
			var path:Array;
			var hasDuplicates:Boolean = (fillStyleIdx0 != 0 && fillStyleIdx1 != 0);
			if (fillStyleIdx0 != 0) {
				path = fillEdgeMap[fillStyleIdx0] as Array;
				if(path == null) { path = fillEdgeMap[fillStyleIdx0] = new Array(); }
				for (var j:int = subPath.length - 1; j >= 0; j--) {
					path.push(subPath[j].reverseWithNewFillStyle(fillStyleIdx0));
				}
			}
			if (fillStyleIdx1 != 0) {
				path = fillEdgeMap[fillStyleIdx1] as Array;
				if(path == null) { path = fillEdgeMap[fillStyleIdx1] = new Array(); }
				appendEdges(path, subPath);
			}
			if (lineStyleIdx != 0) {
				path = lineEdgeMap[lineStyleIdx] as Array;
				if(path == null) { path = lineEdgeMap[lineStyleIdx] = new Array(); }
				appendEdges(path, subPath);
			}
		}
		
		protected function exportFillPath(handler:IShapeExporter):void {
			var path:Array = createPathFromEdgeMap(fillEdgeMap);
		...
		protected function exportLinePath(handler:IShapeExporter):void {
			var path:Array = createPathFromEdgeMap(lineEdgeMap);
			var pos:Point = new Point(Number.MAX_VALUE, Number.MAX_VALUE);
		...
		protected function createPathFromEdgeMap(edgeMap:Dictionary):Array {
			...
			for(var i:uint = 0; i < styleIdxArray.length; i++) {
				appendEdges(newPath, edgeMap[styleIdxArray[i]] as Array);
			}
			return newPath;
		}
		...
		protected function cleanEdgeMap(edgeMap:Dictionary):void {
			for(var styleIdx:String in edgeMap) {
				var subPath:Array = edgeMap[styleIdx] as Array;
				if(subPath && subPath.length > 0) {
					var idx:uint;
					var prevEdge:IEdge;
					var tmpPath:Array = new Array();
		...
		protected function createCoordMap(path:Array):void {
		...
		protected function appendFillStyles(v1:Array, v2:Array):void {
			for (var i:uint = 0; i < v2.length; i++) {
				v1.push(v2[i]);
			}
		}
		
		protected function appendLineStyles(v1:Array, v2:Array):void {
			for (var i:uint = 0; i < v2.length; i++) {
				v1.push(v2[i]);
			}
		}

		protected function appendEdges(v1:Array, v2:Array):void {
			for (var i:uint = 0; i < v2.length; i++) {
				v1.push(v2[i]);
			}
		}
